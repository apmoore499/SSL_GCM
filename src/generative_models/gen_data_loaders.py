...
class SSLDataModule_Unlabel_X(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        target_x = odf[odf.type.isin(['labelled','unlabelled'])][feature_cols].values
        ...
        validation_x=odf[odf.type.isin(['validation'])][feature_cols].values
    ...
    def train_dataloader(self):
        return DataLoader(self.data_train, batch_size=self.batch_size,num_workers=NUM_WORKERS,shuffle=True)


    def val_dataloader(self):
        return DataLoader(self.data_validation, batch_size=1,num_workers=NUM_WORKERS)
...
class SSLDataModule_X_from_Y(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.data_train_labelled=torch.utils.data.TensorDataset(torch.tensor(target_x_labelled.values),
        ...
        self.data_train_unlabelled=torch.utils.data.TensorDataset(torch.tensor(target_x_unlabelled.values),
        ...
        #set_trace()
        vfeat_target=torch.tensor(val_target_x.values).unsqueeze(0)
        ...
        tfeat_target=torch.tensor(target_x_trans.values).unsqueeze(0)
...
class SSLDataModule_Y_from_X(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        #convert to numpy
        x_lab, y_lab = x_lab.values, y_lab.values
        ...
        #convert to numpy
        x_unlab, y_unlab = x_unlab.values, y_unlab.values
        #convert to torch tensor
        ...
        #convert to numpy
        x_validation, y_validation = x_validation.values, y_validation.values
        #convert to torch tensor
...
class SSLDataModule_X2_from_Y_and_X1(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.data_train_labelled=torch.utils.data.TensorDataset(torch.tensor(target_x_labelled.values),
                                                           torch.tensor(conditional_x_labelled.values),
                                                           torch.tensor(y_labelled_oh))
        ...
        self.data_train_unlabelled = torch.utils.data.TensorDataset(torch.tensor(target_x_unlabelled.values),
                                                                    torch.tensor(conditional_x_unlabelled.values),
                                                                    torch.tensor(y_resampled_oh))
...
class SSLDataModule_X_from_X(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.entire_feature_dataset=torch.utils.data.TensorDataset(torch.tensor(target_x_unlabelled.values),
                                                             torch.tensor(conditional_x_unlabelled.values))
    ...
    def train_dataloader(self):
        unlabelled_loader=torch.utils.data.DataLoader(self.entire_feature_dataset,batch_size=self.unlabelled_batch_size,num_workers=NUM_WORKERS,shuffle=True)
...
class X_dataset_resample_Y(Dataset):
    ...
    def __init__(self, orig_data_df,
        ...
        target_x = all_x[self.tvar_names].values.flatten()

        # set conditional X
        cond_x = all_x[self.cvar_names].values
