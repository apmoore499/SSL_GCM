...
list_of_models = ['FULLY_SUPERVISED_CLASSIFIER',
                  ...
                  'CGAN_GUMBEL_DJ_XCES_SUPERVISED_CLASSIFIER',
                  'SSL_GAN',
                  'TRIPLE_GAN',
                  'SSL_VAE',
                  'VAT',
...
if __name__=='__main__':
    ...
    for k in gaussian_results_dict.keys():
        ...
        for p, k in zip(pd_dicts, fd_list):
            p.columns = [k]
        if len(pd_dicts)>0:
            ulab_df = pd.concat(pd_dicts, axis=1)
        else:
            ulab_df = pd.DataFrame()




        ulab_df.to_excel(writer, sheet_name=sname,startrow=2, startcol=0)
        writer.sheets[sname].write_string(1, 0, 'unlabel_set')
        #get max col width
        idx_max = max([len(str(s)) for s in ulab_df.index.values] + [len(str(ulab_df.index.name))])

        col_widths = [idx_max] + [max([len(str(s)) for s in ulab_df[col].values] + [len(col)]) for col in ulab_df.columns]
        ...
        if EXPORT_LATEK:
            latek_df=copy_legend_rename(ulab_df)
